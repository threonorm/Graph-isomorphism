...
// at the end, V contains the number of FREE vertices (in G1, but that's the same in G2) for each color
// 0 = non comptatible  1 = compatible  2 = compatible and stable
{
  ...
  // now whe should check that the distributions of colors in G1 and G2 are compatible
  ...
  for(i=0 ; i<size ; i++)
  {
    if(colors1[i] != old_colors1[i])
      flag = 1;
    if(iso[i] != -1)
      V[colors1[i]]--; // remember that V must contain the number of FREE vertices for each color !  
  }
